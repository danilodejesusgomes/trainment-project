...
const getQuestionAlternativeTable  = () => db_awsimulate.table('QuestionAlternative');

export async function getQuestionAlternativeDb(
  idQuestionAlternative: number
  ): Promise<QuestionAlternativeDb> {
  const questionAlternative: QuestionAlternativeDb = await getQuestionAlternativeTable()
    .first()
